...
2. **קובץ הטיסות החיצוני (חשיפה טיסות 2026 - ענת)** – מנוהל על ידי ענת (רכזת הטיסות) ומסונכרן מול הקובץ הראשי.
...
---
## ✈️ 3. מבנה הקובץ החיצוני: `חשיפה טיסות 2026 - ענת` (קובץ ענת / עדכוני טיסות)

* **שם הקובץ באמת:** `חשיפה טיסות 2026 - ענת`
* **Spreadsheet ID:** `1UDzPjGWdvjJ1yOQHmORY-itX0yr52BMOkSrdac3t95M`
* **שם הגיליון:** `עדכוני טיסות` (או הגיליון הראשון בקובץ)
* **סטטוס נוכחי:** קיימת שורת כותרות בלבד (ללא שורות נתונים/רשומות).

| עמודה | שם עמודה מדויק בגיליון | תפקיד ומשמעות עסקית |
| :---: | :--- | :--- |
| **A** | `מייל אורח` | מפתח ראשי (מיושר לפי סדר המאסטר) |
| **B** | `שם משפחה` | שם משפחה של האורח |
| **C** | `שם פרטי` | שם פרטי של האורח |
| **D** | `מדינה` | מדינת מוצא |
| **E** | `עיר` | עיר מוצא/מגורים |
| **F** | `הגעה לישראל` | תאריך הגעה לישראל |
| **G** | `יעד הגעה` | יעד / מוצא טיסת הגעה |
| **H** | `חזרה מישראל` | תאריך חזרה מישראל |
| **I** | `יעד חזרה` | יעד טיסת חזרה |
| **J** | `אוכל` | העדפות מזון / כשרות / טבעוני |
| **K** | `מושבים` | העדפות מושב במטוס |
| **L** | `הערות טיסות` | הערות מיוחדות לטיסה |
| **M** | `הערות אירוח` | הערות אירוח שקשורות לטיסות/הגעה |
| **N** | `תמונת דרכון` | קישור/סטטוס תמונת דרכון |
| **O** | `מוכן להצעת כרטיס?(שחר)` | אישור שחר להפקת הצעה |
| **P** | `נשלחה הצעה לכרטיס?(ענת)` | אישור ענת על שליחת הצעה |
| **Q** | `מחיר משוער מההצעה` | הערכת מחיר |
| **R** | `חברת תעופה מההצעה` | שם חברת התעופה המוצעת |
| **S** | `נשלח לאורח?(שחר)` | סטטוס שליחת ההצעה לאורח |
| **T** | `כרטיס אושר ע"י אורח(שחר)` | אישור האורח על הכרטיס |
| **U** | `נשלח כרטיס סופי?(ענת)` | סטטוס שליחת כרטיס סופי ע"י ענת |
| **V** | `נשלח לאורח הסכם דמי ביטול(שחר)` | שליחת הסכם דמי ביטול |
| **W** | `נשלח הסכם דמי ביטול חתום(שחר)` | קבלת הסכם חתום |
| **X** | `נשלח כרטיס סופי לאורח?(שחר)` | שליחת כרטיס סופי לאורח |
| **Y** | `לינק כרטיס סופי` | קישור לקובץ הכרטיס |
| **Z** | `מחיר כרטיס סופי` | מחיר סופי של הכרטיס |
| **AA** | `דמי ביטול` | עלות דמי ביטול |
| **AB** | `דמי שינוי (+ הפרשי מחיר אם יהיו)` | עלות דמי שינוי/הפרשים |
| **AC** | `סה"כ כולל בדולרים` | סך הכל עלות ב-USD ($) |
| **AD** | `סה"כ כולל בשקלים` | סך הכל עלות ב-ILS (₪) |
| **AE** | `סופי` | סטטוס סופי סגור |
